...
use App\Http\Controllers\Student\ApplController as StudentApplication;
...
class FieldPreloads {
    ...
    public static function getEnrolleeStatuses(int $mode, bool $is_valid=true, int $ays_offset=0, int $new_acedemic_status=0) {
        ...
        // if(CLHF::AUTH_RoleAuthorized(['Administrator'], CLHF::AUTH_UserID())) {
        //     dd($ays_offset);
        // }
        ...
        foreach($output as $key=>$val) {
            ...
            elseif($mode === 1) {
                ...
                if(in_array($new_acedemic_status, [6,7,11]) || in_array($val['id'], [7,5,12])) {
                    ...
                    elseif($new_acedemic_status === 6 && in_array($val['id'], [12])) {
                        $output2[] = $val;
                        break;
                    }
                } else {
