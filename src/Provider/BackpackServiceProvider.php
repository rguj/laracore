...
use Rguj\Laracore\Library\LBP\CrudPanel\CrudPanel;
...
class BackpackServiceProvider extends ServiceProvider
{
    ...
    protected $lbp_path = __DIR__.'/../../../../backpack/crud/src';
    ...
    public function boot(Router $router)
    {
        ...
        $this->loadTranslationsFrom(realpath($this->lbp_path.'/resources/lang'), 'backpack');
        $this->loadConfigs();
    ...
    {
        // load the macros
        include_once $this->lbp_path.'/macros.php';

        // Bind the CrudPanel object to Laravel's service container
        $this->app->scoped('crud2', function ($app) {
    ...
    {
        $error_views = [$this->lbp_path.'/resources/error_views' => resource_path('views/errors')];
        $backpack_views = [$this->lbp_path.'/resources/views' => resource_path('views/vendor/backpack')];
        $backpack_public_assets = [$this->lbp_path.'/public' => public_path()];
        $backpack_lang_files = [$this->lbp_path.'/resources/lang' => app()->langPath().'/vendor/backpack'];
        $backpack_config_files = [$this->lbp_path.'/config' => config_path()];

        // sidebar content views, which are the only views most people need to overwrite
        $backpack_menu_contents_view = [
            $this->lbp_path.'/resources/views/base/inc/sidebar_content.blade.php'      => resource_path('views/vendor/backpack/base/inc/sidebar_content.blade.php'),
            $this->lbp_path.'/resources/views/base/inc/topbar_left_content.blade.php'  => resource_path('views/vendor/backpack/base/inc/topbar_left_content.blade.php'),
            $this->lbp_path.'/resources/views/base/inc/topbar_right_content.blade.php' => resource_path('views/vendor/backpack/base/inc/topbar_right_content.blade.php'),
        ];
        $backpack_custom_routes_file = [$this->lbp_path.$this->customRoutesFilePath => base_path($this->customRoutesFilePath)];

        // calculate the path from current directory to get the vendor path
        $vendorPath = dirname($this->lbp_path, 3);
    ...
    public function setupRoutes(Router $router)
    {
        ...
        $routeFilePathInUse = $this->lbp_path.$this->routeFilePath;
    ...
    public function loadViewsWithFallbacks()
    {
        ...
        $this->loadViewsFrom(realpath($this->lbp_path.'/resources/views/base'), 'backpack');
        $this->loadViewsFrom(realpath($this->lbp_path.'/resources/views/crud'), 'crud');
    }

    protected function mergeConfigFromOperationsDirectory()
    {
        $operationConfigs = scandir($this->lbp_path.'/config/backpack/operations/');
        ...
        foreach ($operationConfigs as $configFile) {
            $this->mergeConfigFrom(
                $this->lbp_path.'/config/backpack/operations/'.$configFile,
    ...
    public function loadConfigs()
    {
        ...
        $this->mergeConfigFrom($this->lbp_path.'/config/backpack/crud.php', 'backpack.crud');
        $this->mergeConfigFrom($this->lbp_path.'/config/backpack/base.php', 'backpack.base');
    ...
    {
        require_once $this->lbp_path.'/helpers.php';
    }
